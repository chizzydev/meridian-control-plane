...
export const P05_USER_SECURITY_PATH =
  `/v2/security/user?name=${encodeURIComponent(DEMO_FIXTURE_USERNAME)}` as const;

export const P05_LOGIN_PATH = "/login" as const;

interface JsonRecord {
  readonly [key: string]: unknown;
}

export interface P05UserSnapshot {
...
export interface P05LoginProbe {
...
export class P05UserAuthorityDeniedError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "P05UserAuthorityDeniedError";
  }
}

export class P05UserMutationRejectedError extends Error {
  constructor(
    readonly status: number,
    message: string,
  ) {
    super(message);
    this.name = "P05UserMutationRejectedError";
  }
}

export class P05UserMutationUnknownAfterDispatchError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "P05UserMutationUnknownAfterDispatchError";
  }
}
...
function resultObject(value: unknown): JsonRecord {
  if (!isRecord(value)) {
    throw new Error("IRIS P05 response is not an object.");
  }
...
function integerValue(value: unknown): number {
  ...
  throw new Error("IRIS P05 integer representation is unsupported.");
}
...
function stringArray(value: unknown): readonly string[] {
    ...
    return Object.freeze(
      value.map((item) => {
        if (typeof item !== "string") {
          throw new Error("IRIS P05 string list contains a non-string value.");
        }
  ...
  throw new Error("IRIS P05 string-list representation is unsupported.");
}
...
export async function readP05User(input: {
  readonly apiBaseUrl: string;
  readonly accessToken: string;
  readonly fetchImpl?: typeof fetch;
}): Promise<P05UserSnapshot | null> {
  const fetchImpl = input.fetchImpl ?? fetch;
  const response = await fetchImpl(
    input.apiBaseUrl.replace(/\/+$/, "") + P05_USER_SECURITY_PATH,
    {
      method: "GET",
      cache: "no-store",
  ...
  if (response.status === 401 || response.status === 403) {
    throw new P05UserAuthorityDeniedError(
...
export async function probeP05Login(input: {
  readonly apiBaseUrl: string;
  readonly username: typeof DEMO_FIXTURE_USERNAME;
  readonly password: string;
  readonly fetchImpl?: typeof fetch;
}): Promise<P05LoginProbe> {
  if (input.password.length === 0) {
    throw new Error("P05 login probe requires the in-memory synthetic credential.");
  }

  const fetchImpl = input.fetchImpl ?? fetch;
  const response = await fetchImpl(
    input.apiBaseUrl.replace(/\/+$/, "") + P05_LOGIN_PATH,
    {
      method: "POST",
      cache: "no-store",
      headers: {
  ...
  if (response.status !== 200) {
    throw new Error(
      `P05 login probe returned unexpected HTTP ${response.status}. Body=${text.slice(0, 500)}`,
    );
  }

  const result = resultObject(parsedJson(text, "P05 successful login probe"));
  ...
  if (accessToken.length === 0 || refreshToken.length === 0) {
    throw new Error("P05 successful login probe did not return both tokens.");
  }
...
export async function putP05UserEnable(input: {
  readonly apiBaseUrl: string;
  readonly accessToken: string;
  readonly fetchImpl?: typeof fetch;
}): Promise<Readonly<{
  ...
  try {
    ...
    throw new P05UserMutationUnknownAfterDispatchError(
  ...
  if (response.status === 401 || response.status === 403) {
    throw new P05UserMutationRejectedError(
      response.status,
      `P05 authority rejected by SysAdmin API with HTTP ${response.status}.`,
    );
  }
  ...
  if (!response.ok) {
    throw new P05UserMutationRejectedError(
      response.status,
      `Official SysAdmin PUT /v2/security/user rejected P05 with HTTP ${response.status}. Body=${text.slice(0, 500)}`,
    );
  }
